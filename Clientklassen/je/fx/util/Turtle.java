 ...
 * 4.01 2020.02.09 bugfix setTurtleX/Y/W
 * 4.02 2020.02.11 setTurtleX/Y/W without animation

 * This class is a turtle component for simple graphic programming.
 *
 * @see <a href="http://www.javaeditor.org">
 * @author Gerhard Röhner
 * @version 4.01 2020.02.09
 */
...
public class Turtle extends Pane {
    ...
    if (seqtr.getStatus() != Animation.Status.RUNNING) {
      ...
      if (animated) {
        ...
        if (isDrawing) {
          ...
          if (maxSpeed)
            keyFrame = new KeyFrame(Duration.millis(1), xTurtle, yTurtle);
          else
  ...
  private void rotate(double angle) {
    if (animated && visibleImage) {
      ...
      if (maxSpeed) 
        rt = new RotateTransition(Duration.millis(1), turtleImage);
      else 
        rt = new RotateTransition(Duration.millis(speed), turtleImage);
  ...
  /**
   * Gets the background color of the Turtle.
   * <P>
